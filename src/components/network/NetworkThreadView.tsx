...
const NetworkThreadView = forwardRef<ThreadViewRef, Props>(
  ({ threadId, currentUserId }, ref) => {
    ...
    return (
      <div className="flex flex-col gap-4 max-h-[400px] overflow-y-auto px-3 bg-neutral-50 pt-2 pb-2 rounded-xl border border-neutral-200 shadow-sm">
        {messages.map((m) => {
              ...
              <div
                className={`text-sm leading-relaxed px-4 py-2 rounded-2xl max-w-[65%] break-words shadow-sm ${
                  isMe
                    ...
                    : "bg-gray-200 text-neutral-900 shadow-sm"
                }`}
              >
                {!isMe && (
